...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    let cp_program_str = config.get("Global", "cp_program").unwrap();
    if cp_program_str.is_empty() {
        panic!("cp_program must not be empty");
    }
    let cp_program = Pubkey::from_str(&cp_program_str).unwrap();
...
fn main() -> Result<()> {
    ...
    let program = anchor_client.program(pool_config.cp_program)?;

    let opts = Opts::parse();
    match opts.command {
        CpCommands::InitializePool {
            mint0,
            mint1,
            init_amount_0,
            init_amount_1,
            ...
            let initialize_pool_instr = initialize_pool_instr(
                ...
                cp_swap::create_pool_fee_reveiver::id(),
                init_amount_0,
                init_amount_1,
                open_time,
            )?;
        ...
        CpCommands::Deposit {
            ...
            let pool_state: cp_swap::states::PoolState = program.account(pool_id)?;
            // load account
            ...
            // calculate amount
            let results = cp_swap::curve::CurveCalculator::lp_tokens_to_trading_tokens(
                u128::from(lp_token_amount),
                u128::from(pool_state.lp_supply),
                u128::from(total_token_0_amount),
                u128::from(total_token_1_amount),
                cp_swap::curve::RoundDirection::Ceiling,
            )
            .ok_or(cp_swap::error::ErrorCode::ZeroTradingTokens)
            .unwrap();
            println!(
        ...
        CpCommands::SwapBaseIn {
            ...
            let amm_config_state = deserialize_anchor_account::<cp_swap::states::AmmConfig>(
            ...
            let (
                ...
                (
                    cp_swap::curve::TradeDirection::ZeroForOne,
                ...
                (
                    cp_swap::curve::TradeDirection::OneForZero,
                    total_token_1_amount,
                    total_token_0_amount,
                    user_input_token,
                    spl_associated_token_account::get_associated_token_address(
            ...
            let result = cp_swap::curve::CurveCalculator::swap_base_input(
        ...
        CpCommands::SwapBaseOut {
            ...
            let result = cp_swap::curve::CurveCalculator::swap_base_output(
        ...
        CpCommands::DecodeInstruction { instr_hex_data } => {
            handle_program_instruction(&instr_hex_data, InstructionDecodeType::BaseHex)?;
        }
        CpCommands::DecodeEvent { log_event } => {
            handle_program_log(
                &pool_config.cp_program.to_string(),
                &log_event,
                false,
            )?;
        }
        CpCommands::DecodeTxLog { tx_id } => {
            ...
            // decode logs
            parse_program_event(&pool_config.cp_program.to_string(), meta.clone())?;
        }
    }
    Ok(())
}
